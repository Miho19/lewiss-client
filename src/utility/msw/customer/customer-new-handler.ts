...
const customerNewHandler = [
  http.post<{}, CustomerCreateType, CustomerType>(toURL('/customer'), async ({ request }) => {
    ...
    try {
      ...
      const familyNameFound = testCustomers.find((c) =>
      ...
      const mobileFound = testCustomers.find((c) =>
        c.mobile.localeCompare(parseData.mobile, undefined, { sensitivity: 'base' }),
      );
      if (mobileFound) return HttpResponse.json(mobileFound);

      const emailFound = testCustomers.find((c) =>
        c.email.localeCompare(parseData.email, undefined, { sensitivity: 'base' }),
      );
